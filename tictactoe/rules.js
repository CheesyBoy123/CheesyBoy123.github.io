...
function begin_play(){
	//game has already begun don't do anything
	if(game_started()) {
	...
	document.getElementById("turn_info").innerHTML = "Turn for: " + (whose_move() ? "X" : "O");
}
/*
TODO - Rule 2
...
function validateMove() {
	//Go across all rows
	for(j = 0; j < 3; j++) {
		//If the space is empty there can't be a winning move here. This is for the edge case of all 3 being empty moves.
		if(board_state[3*j] == -1) continue;
		//Someone one going across
		if(board_state[3*j] == board_state[1 + 3*j] && board_state[1 + 3*j] == board_state[2 + 3*j]) {
			alert("The winner is " + (board_state[3*j] ? "X" : "O"));
			reset_play();
			return;
		}
	}
	//Go down each column
	for(i = 0; i < 3; i++) {
		//If there is an empty space there can't be a winner move here. This is for the edge case of all 3 being empty.
		if(board_state[i] == -1) continue;
		//Someone one going across
		if(board_state[i] == board_state[i + 3] && board_state[i + 3] == board_state[i + 6]) {
			alert("The winner is " + (board_state[i] ? "X" : "O"));
			reset_play();
			return;
		}
	}
	...
	if(board_state[0] == board_state[4] && board_state[4] == board_state[8] && board_state[0] != -1) {
		alert("The winner is " + (board_state[0] ? "X" : "O"));
		reset_play();
		return;
	}

	if(board_state[2] == board_state[4] && board_state[4] == board_state[6] && board_state[2] != -1) {
		alert("The winner is " + (board_state[2] ? "X" : "O"));
		reset_play();
		return;
	}
